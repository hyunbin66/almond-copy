...
(function () {
  ...
  window["CAFE24-COUNTRY"].changeTextCode = function (LANGUAGE, SCOPE) {
    ...
    if (getUrlParameter("langcode"))
      window["CAFE24-COUNTRY"].code = getUrlParameter("langcode");
...
function gridCompatibilityforIE(contextSelector) {
  ...
  $gridContext.css("display", "-ms-grid");
  ...
  if (hasGridTemplate) {
    ...
    if (splitGridTemplate.row.match("repeat")) {
      ...
      msGridTemplateStyle =
        "-ms-grid-rows:" + changeRepeatStyle(repeatCount[0], repeatCount[1]);
    } else {
    ...
    $gridContext.attr(
      "style",
      $gridContext.attr("style") + ";" + msGridTemplateStyle
    );
    ...
    if (splitGridTemplate.col.match("repeat")) {
      ...
      msGridTemplateStyle =
        "-ms-grid-columns:" + changeRepeatStyle(repeatCount[0], repeatCount[1]);
    } else {
  ...
  $gridItem.each(function (idx, element) {
    ...
    if (hasGridArea) {
      ...
      msGridAreaStyle =
        ...
        hasGridArea[1] +
        "; -ms-grid-row-span: " +
        (hasGridArea[2] - hasGridArea[0]) +
        "; -ms-grid-column-span: " +
        (hasGridArea[3] - hasGridArea[1]) +
        "";
