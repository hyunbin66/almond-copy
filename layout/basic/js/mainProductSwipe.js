...
var $S = {
  ...
  setBlock: function () {
    // set block num
    this.iPageBlock =
      this.sMode == "multi" ? this.iLine * this.aGrid[this.sGrid] : 1;
  },
  ...
  resetGrid: function () {
    for (var sKey in this.aGrid) {
      if (this.$productModule.hasClass(sKey) === true) {
        this.$productModule.empty().removeClass(sKey);
      }
    }
  },
  ...
  generateMulti: function () {
    ...
    this.$productList.each(function () {
      $S.aElement.push($(this).clone(true));
    });
    ...
    for (var i = 0, k = 1, j = 0; i < this.aElement.length; i++, k++) {
      ...
      var $template =
        j == 0
          ? $("<li>", { html: $("<ul>", { class: this.sGrid }) })
          : $("<li>", {
              html: $("<ul>", { class: this.sGrid }),
              css: { display: "none" },
            });
  ...
  makePagenate: function () {
    ...
    aBtn.push(
      '<button type="button" class="prev" onclick="$' +
        this.sModuleSwipe +
        "_slider_" +
        this.iActive +
        '.prev();return false;"><span>이전 리스트</span></button>'
    );
    aBtn.push(
      '<span id="' +
        this.sModule +
    ...
    aBtn.push(
      '<button type="button" class="next" onclick="$' +
        ...
        "_slider_" +
        this.iActive +
        '.next();return false;"><span>다음 리스트</span></button>'
    );
    aBtn.push("</div>");
  ...
  load: function () {
    try {
      var aSwipeVars = [],
        $swipe = document.getElementById(
          "" + this.sModule + "-slider-" + this.iActive + ""
        ),
        $button = $(
          "#" + this.sModule + "-swipe-button-" + this.iActive + ""
        ).find("button"),
        $now =
          this.iActive > 0
            ? $("." + this.sModule + "-" + this.iActive + "")
      ...
      aSwipeVars.push(
        "$" +
          ...
          "_slider_" +
          this.iActive +
          " = new SwipeClient($swipe, {"
      );
      aSwipeVars.push("    startSlide: " + this.iStart + ",");
      aSwipeVars.push("    callback: function(e, pos) {");
      aSwipeVars.push(
        "        if ($S.bSlider === true) {  $now.text(pos + 1); }"
      );
      aSwipeVars.push("        if (pos > ($S.iLimit - 1)) {");
      aSwipeVars.push("            $button.each(function(index){");
      aSwipeVars.push(
        "                $S.iLimit - 1 != index ? $(this).removeClass('selected') : $(this).addClass('selected');"
      );
      aSwipeVars.push("            });");
      aSwipeVars.push("        } else {");
      aSwipeVars.push("            $button.each(function(index){");
      aSwipeVars.push(
        "                pos == index ? $(this).addClass('selected') : $(this).removeClass('selected');"
      );
      aSwipeVars.push("             });");
      aSwipeVars.push("         }");
      aSwipeVars.push("    }");
